...
class WeekData {
	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false, isFreeplay:Null<Bool> = false)
	{
		weeksList = [];
		weeksLoaded.clear();
		#if MODS_ALLOWED
			...
			for (j in 0...directories.length) {
				...
				if(!weeksLoaded.exists(sexList[i])) {
					...
					if(week != null) {
						...
						if(weekFile != null && (isStoryMode == null || (isStoryMode && !weekFile.hideStoryMode) || (isFreeplay && !weekFile.hideFreeplay))) {
	...
	private static function addWeek(weekToCheck:String, path:String, directory:String, i:Int, originalLength:Int)
	{
		if(!weeksLoaded.exists(weekToCheck))
		{
			...
			if(week != null)
			{
				...
				if((PlayState.isStoryMode && !weekFile.hideStoryMode) || (PlayState.isFreeplay && !weekFile.hideFreeplay))
				{
