...
public var camOtherShaders:Array<ShaderEffect> = [];
	...
	override public function create()
	{
		...
		{
			// case 'barrier':
			// 	barrier = new FlxSprite().makeGraphic(FlxG.width, FlxG.height, FlxColor.fromRGB(24, 24, 24));
			// 	if (defaultCamZoom < 1)
			// 	{
			// 		barrier.scale.scale(1 / defaultCamZoom);
			// 	}
			// 	barrier.scrollFactor.set();
			// 	add(barrier);
			// the unused - hooovie
		...
		if(ClientPrefs.downScroll) timeTxt.y = FlxG.height - 50;
		...
		if(SONG.song == 'house')
			{
				scoreTxt = new FlxText(200, healthBarBG.y + 36, FlxG.width, "", 16);
		...
		// 	{
		// 		case 'renovation':
		// 			LoadingState.loadAndSwitchState(new RenovationWarningState());
		// 	}
		// }
		// broken code, mersak sprawil ze powiadomienie sie zapetla
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'ugh' | 'guns' | 'stress':
					tankIntro();
				case 'district':
					startVideo('districtcut');
				case 'renovation':
					startVideo('renovationcut');
				case 'dispute':
					startVideo('disputecut');
				case 'breathe':
					startVideo('breathecut');
				default:
					startCountdown();
  ...
  public function clearShaderFromCamera(cam:String){
	...
	function startAndEnd()
	{
		...
		if(SONG.song == "renovation") MusicBeatState.switchState(new RenovationWarningState());
		if(SONG.song != "renovation")startCountdown();
	}
	...
	public function startCountdown():Void
	{
		if(SONG.song == 'house')
			{
				iconP1.changeIcon('icon-awesome');
				iconP2.changeIcon('icon-awesome_red');
				timeTxt.visible = false;
			}
if(SONG.song == 'fundament')
	{
	...
	public function updateScore(miss:Bool = false)
	{
		...
		if(SONG.song == 'renovation' && !instakillOnMiss && songMisses < 3)
			{
				kaszana = new FlxTextFormat(0xFF3CFF00);
				var green = new FlxTextFormatMarkerPair(kaszana, "@");
				scoreTxt.applyMarkup('Score: ' + songScore
				+ ' |@ Misses: ' + songMisses
				+ '@ | Rating: ' + ratingName
				+ (ratingName != '?' ? ' (${Highscore.floorDecimal(ratingPercent * 100, 2)}%) - $ratingFC' : ''), [green]);
			}
			else if(SONG.song == 'renovation' && instakillOnMiss && songMisses < 3)
				{
...
else if(SONG.song == 'renovation' && songMisses < 6)
	{
	...
	else if(SONG.song == 'renovation' && songMisses < 9)
		{
		...
		else if(SONG.song == 'renovation' && songMisses < 12)
			{
			...
			else if(SONG.song == 'renovation' && songMisses == 14)
				{
					kaszana = new FlxTextFormat(0xFF9B0000);
					var green = new FlxTextFormatMarkerPair(kaszana, "@");
					scoreTxt.applyMarkup('Score: ' + songScore
...
// if(SONG.song == 'barrier')
// 	{popupWindow(326, 290, 1250, 500, 'bf', true);
// popupWindow(326, 590, 500, 250, 'house', false);}
// the unused - hooovie
		...
		switch(curStage)
		{
			...
			case 'whitescreen':
				trace('dnb expanded');
						...
						black = new FlxSprite(-600, -600).makeGraphic(FlxG.width * 10, FlxG.height * 10, 0xFF000000);
						add(black);
						obrazek1 = new FlxSprite(-125, 100).loadGraphic(Paths.image('gallery/fundamentgallery/5'));
...
// 	function popupWindow(customWidth:Int, customHeight:Int, ?customX:Int, ?customY:Int, ?customName:String, ?Bf:Bool) {
...
//         spr.graphics.beginBitmapFill(barrier.pixels, m);
//         spr.graphics.drawRect(0, 0, barrier.pixels.width, barrier.pixels.height);
...
// 	else
// 			{ 
// 				bfWin.graphics.beginBitmapFill(boyfriend.pixels, m);
...
// 			// windowDad.stage.addChild(spr);
// 				windowDad.stage.addChild(bfScrollWin);
// 				bfScrollWin.addChild(bfWin);
// 				bfScrollWin.scaleX = 0.7;
// 				bfScrollWin.scaleY = 0.7;
...
// the unused
	...
	override public function update(elapsed:Float)
	{
		if(SONG.player1 == 'bfhouse')
			...
			if(SONG.player1 == 'pico-player' || SONG.player1 == 'pico-player-dom' || SONG.player1 == 'DistrictPico')
				...
				if(SONG.player1 == 'pico-player'|| SONG.player1 == 'DistrictPico')
					{
						GameOverSubstate.characterName = 'picodead';
					}
					if(SONG.player1 == 'pico-player-dom')
						{
							GameOverSubstate.characterName = 'picodeadhouse';
						}
						if(SONG.song == 'house')
							{
								GameOverSubstate.deathSoundName = 'Fart with reverb sound effect';
							}
				// if(SONG.song == 'barrier')
				// 	{
        ...
        if (BFFrame == null || BFFrame.frame == null) return; // prevents crashes (i think???)
	...
	{
		if(SONG.song == 'fundament' && curBeat < 64)
			{
			}
			else
				{
		...
		switch(eventName) {
			...
			case 'BG Freaks Expression':
				...
				case 'House Throw':
					...
					switch(value2) {
						case 'bf':
							boyfriend.visible = aliengej;
						case 'gf':
							gf.visible = aliengej;
						case 'dad':
                            dad.visible = aliengej;
						default:
							boyfriend.visible = aliengej;
					}
	...
	{
		penis = false;
		if(isStoryMode && SONG.song == 'renovation'){
			Saves.renovationLock = 'unlocked';
			Saves.saveShit();
		}
		if(SONG.song == 'fundament'){
			Saves.fundamentLock = 'unlocked';
			Saves.saveShit();
		}
		if(SONG.song == 'multiversus'){
			Saves.multiversusLock = 'unlocked';
			Saves.saveShit();
		}
		finishSong(false);
	}
	...
	public function endSong():Void
	{
		...
		if(SONG.song == 'renovation') {
			if (isStoryMode) {
                var filetxt = File.write("CAN_YOU_HEAR_ME.txt", true);

                filetxt.writeString("psst! Hey, you there! Yes, you! C̷̮̹͉͕̋́͐͑̌̌͆ą̷̮̣̠͑̎̃̾̌͂͝ṋ̴͎̮͇̂̎̐͆̈́̚ ̷̧̅̽̈́̓͠͝ẙ̵̨̪͓̊͌͌̾o̴͚͍̔̀ȕ̵͓̟̔̆̀̑͝ hear me?... Ö̶̤̲́͊k̶̳͔͆ ̵̠̗͂g̶̢̣͠o̶͖̽͝ͅo̶͎͆̈́ḋ̷͕̭̋.̶̱̥́ I am the R̵̘͓̳̘�
̾͗̿̊̈͜͝͠ͅͅd̷̛̛͈͍̠̞͈̖͖̖͓̠̪̘̫̞͙̤̦̩̯͓̽͛̽̌͊̀͑̀̒̂͆̔̓̍̄̎̑̅̉̊͂̈́͂͛̐̍͊͂̈͊́̓̅́̄͘̚͠ͅa̸̡̧̨͖͍̣̖̬̟̠͕̣̻̦͍͚̩̣͓͚̹̙̩̦̬̤̦̬̗͇̙̝̩̹͎͔͔̱͖̖̬̯̔͛̍̋̈̇̔̃̽̈͌̉̎̓̾̉͑̃̄̈́͘͜͜͝͝m̸̡̡̛͔̼͕̣̺̱̬̝̩̻̳̩̯̘̅̃̑̆̓̀̈̾̊͒̑̐̎̿̃̒͆́̑̉͘̚͝ͅȩ̸̛̛͚͓̻͎̹͓̮͎͈̝̝͔͔̥̳̥͕̻͙͇̖̬͇̱̮̖̠̝̺̪̣̜̝̬̇̂͒̔́̃̃̀̽̎͒̾̈͊̇͘͝͠͠n̴̢̛̺̺̖͚̘̯̩̝̣̺̣̠̣̘̭̥͎͎̗̥̳̺͕͔̩̹̔̐̓͐̉͗͊̓̈́̊̈́̑̈͋̍̋̽̓̈́̓͜͝͝ͅt̵̢̧̡̢̨̛̛̬͉̯̙̝̬͙̳͇̪̦̗̙̮͖͔͇̪̫͉̮̹̘͕̟͍̺̭͓̺̘͚͇̯͓͎̥͇̠͙͍̮̘̘̍̄̈́͊͆̈̆͗̌͂͂̈́̏͑̿̽̂̑̈́̀́̔͒̏̎͆̎̐̃̀̊̀̇̂̅̊̐͘͘͘̕̚͠͠   ");
	...
	override function beatHit()
	{
		...
		if(SONG.song == 'fundament' && curBeat == 16)
			{
				var obrazek2 = new FlxSprite(-125, 100).loadGraphic(Paths.image('gallery/fundamentgallery/4'));
			...
			if(SONG.song == 'fundament' && curBeat == 32)
				{
					var obrazek3 = new FlxSprite(-125, 100).loadGraphic(Paths.image('gallery/fundamentgallery/3'));
				...
				if(SONG.song == 'fundament' && curBeat == 48)
					{
						var obrazek4 = new FlxSprite(-125, 100).loadGraphic(Paths.image('gallery/fundamentgallery/1'));
					...
					if(SONG.song == 'fundament' && curBeat == 56)
						{
							why = new FlxText(0, 0, FlxG.width, "WHY", 20);
							why.setFormat(Paths.font("vcr.ttf"), 128, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
						...
						if(SONG.song == 'fundament' && curBeat == 58)
							{
								why.text = "DID YOU";
				
							}
							if(SONG.song == 'fundament' && curBeat == 59)
								{
									why.text = "REPLACE";
					
								}
								if(SONG.song == 'fundament' && curBeat == 62)
									{
										why.text = "ME?";
									...
									if(SONG.song == 'fundament' && curBeat == 511)
										{
											SexyTween = FlxTween.tween(dadGroup, {alpha: 0},23.77, {
											});
